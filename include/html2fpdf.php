...
- CSS + align = bug!
- obs: para textos de mais de 1 p醙ina, talvez tenha que juntar varios $texto_artigo
...
OBS0-3: //! is a special comment to be used with source2doc.php, a scrpit I created
in order to generate the doc on the site html2fpdf.sf.net
OBS1:
var $LineWidth;         line width in user unit   to make css thin/medium/thick difference?
...
OBS4: Caso particular ... PDF gerado deve ser o mesmo que o do botao Imprimir
OBS5: Regexp + Perl ([preg]accepts non-greedy quantifiers while PHP[ereg] does not)
...
/* $e == content */
  ...
  if ($this->usecss) $html = $this->ReadCSS($html);
    //Add new supported tag in the long string below
    $html=strip_tags($html,"<font><sup><sub><input><select><option><textarea><title><form><ol><ul><li><h1><h2><h3><h4><pre><b><u><i><a><img><p><br><strong><em><code><th><tr><blockquote><hr><td><tr><table><div>"); //remove all unsupported tags, but the ones inside the long string
...
//                if (!$this->divbegin) $this->x = $this->oldx + $this->GetStringWidth($e) - $this->cMargin;
      ...
      //Start of 'if/elseif's
            ...
            if($this->orderedlist) 
            {  
                  $this->textbuffer[] = array($e,$this->HREF,$this->currentstyle,$this->colorarray,$this->currentfont,$this->SUP,$this->SUB);
          ...
          if($this->tdbegin)
          {
                    $this->cell[$this->row][$this->col]['textbuffer'][] = array($e,$this->HREF,$this->currentstyle,$this->colorarray,$this->currentfont,$this->SUP,$this->SUB);
            ...
            elseif($this->divbegin) $this->textbuffer[] = array($e,$this->HREF,$this->currentstyle,$this->colorarray,$this->currentfont,$this->SUP,$this->SUB);
            elseif($this->pbegin)
            {
          $this->textbuffer[] = array($e,$this->HREF,$this->currentstyle,$this->colorarray,$this->currentfont,$this->SUP,$this->SUB);
    ...
    switch($tag){
      ...
      while (isset($this->cell[$this->row][$this->col])) $this->col++;
        ...
        if ($this->table['nc'] < $this->col+1) $this->table['nc'] = $this->col+1;
            ...
            if (isset($attr['HEIGHT'])) $this->cell[$this->row][$this->col]['h']    = $attr['HEIGHT']/4; //ConvertSize($attr['HEIGHT']);
            ...
            $cs = $rs = 1;
            if (isset($attr['COLSPAN']) && $attr['COLSPAN']>1)  $cs = $this->cell[$this->row][$this->col]['colspan']    = ConvertSize($attr['COLSPAN'],$this->pgwidth);
            if (isset($attr['ROWSPAN']) && $attr['ROWSPAN']>1)  $rs = $this->cell[$this->row][$this->col]['rowspan']    = ConvertSize($attr['ROWSPAN'],$this->pgwidth);
      ...
      $this->SetFontSize(8); 
          break;
        case 'HR': //editado para esta vers鉶
...
//          else $Width = $this->w - $this->lMargin - $this->rMargin;
            ...
            if( isset($attr['CLASS']) or isset($attr['ID']) )
      {
        ...
        break;
            ...
            if($this->divbegin) $this->textbuffer[] = array("\n",$this->HREF,$this->currentstyle,$this->colorarray,$this->currentfont);
            if($this->pbegin) $this->textbuffer[] = array("\n",$this->HREF,$this->currentstyle,$this->colorarray,$this->currentfont);
            $this->Ln(5);
            break;
        case 'P':
        if ($this->tablestart) break; //especial para esta vers鉶
          $this->pbegin=true;
            $this->Ln(6);
            if( $attr['ALIGN'] != '' ) $this->divalign = $align[strtolower($attr['ALIGN'])];
            if( isset($attr['CLASS']) or isset($attr['ID']) )
      ...
      $this->Ln(6);
        ...
        $this->currentfont='courier';
          break;
        case 'TEXTAREA':
//TODO make its content behave like pre contents? edit htmltoolkit ... dont erase \n trim text cehck results
...
///
      ...
      else $this->Write(5,$vetor[0],$aux_x+$dx,$vetor[1],$align); //THE text
      if($vetor[1])
      {
        $this->SetTextColor(0);
        $this->SetStyle('U',false);
...
//paliative fix: if w == pgwidth dont wordwrap
