...
class navigation extends general {
	public function __construct() { // 公用
		...
		$this->tpl_title = T ( "Place holder" );
		$this->navigation_current = 'placeholder';
	...
	public function index() { // 这里是首页
		...
		if (isset ( $_POST )) {
			foreach ( $_POST as $v ) {
				if (isset ( $v ['mlid'] )) {
					$data = array ('weight' => $v ['weight'], 'parentid' => $v ['plid'], 'enabled' => $v ['hidden'] + 0, 'expanded' => $v ['expanded'] + 0 );
	...
	public function post() {
		...
		$data = array ('name' => $this->spArgs ( "name" ), 'position' => $this->spArgs ( "position" ), 'enabled' => $this->spArgs ( "enabled" ) );
